import pandas as pd
ucr_112_dataset_names =['ACSF1','Adiac','ArrowHead','Beef','BeetleFly','BirdChicken','BME','Car','CBF','Chinatown',\
...
'MedicalImages','MiddlePhalanxOutlineAgeGroup','MiddlePhalanxOutlineCorrect','MiddlePhalanxTW','MixedShapesRegularTrain',\
'MixedShapesSmallTrain','MoteStrain','NonInvasiveFetalECGThorax1','NonInvasiveFetalECGThorax2','OliveOil','OSULeaf',\
'PhalangesOutlinesCorrect','Phoneme','PigAirwayPressure','PigArtPressure','PigCVP','Plane','PowerCons',\
'ProximalPhalanxOutlineAgeGroup','ProximalPhalanxOutlineCorrect','ProximalPhalanxTW','RefrigerationDevices',\
...
'Strawberry','SwedishLeaf','Symbols','SyntheticControl','ToeSegmentation1','ToeSegmentation2','Trace','TwoLeadECG',\
'TwoPatterns','UMD','UWaveGestureLibraryAll','UWaveGestureLibraryX','UWaveGestureLibraryY','UWaveGestureLibraryZ',\
'Wafer','Wine','WordSynonyms','Worms','WormsTwoClass','Yoga']
development_datasets=[
    'Beef','BirdChicken','Car','CricketX','CricketY','CricketZ','DistalPhalanxTW','ECG5000','FiftyWords','Fish',
'FordA','FordB','Haptics','Herring','InsectWingbeatSound','ItalyPowerDemand','LargeKitchenAppliances','Lightning7',
'Meat','MedicalImages','MiddlePhalanxOutlineAgeGroup','MiddlePhalanxOutlineCorrect','OliveOil','OSULeaf','Phoneme',
'Plane','ProximalPhalanxOutlineCorrect','ProximalPhalanxTW','ScreenType','ShapeletSim','Strawberry','SwedishLeaf',
'SyntheticControl','ToeSegmentation1','Trace','UWaveGestureLibraryY','Wafer','WordSynonyms','Worms','Yoga'
]
...
def read_dataset(inputdata_dir,dataset_name):
    datasets_dict = {}
    df_train = pd.read_csv(inputdata_dir + dataset_name+'/'+dataset_name + '_TRAIN.tsv', sep='\t',dtype=float, header=None)
    df_test = pd.read_csv(inputdata_dir + dataset_name + '/'+dataset_name +'_TEST.tsv', sep='\t', dtype=float,header=None)
    y_train = df_train.values[:, 0]
    y_test = df_test.values[:, 0]
    x_train = df_train.drop(columns=0).values
    x_test = df_test.drop(columns=0).values
    datasets_dict[dataset_name] = (x_train, y_train, x_test,
                                   y_test)
